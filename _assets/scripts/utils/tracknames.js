/*
    Shared track name data for Mario Kart games, along with a helper to resolve
    track names that differ between US and PAL regions to whichever variant
    matches the site's locale setting.
*/
...
function findRegionPair(name) {
    for (const pool of Object.values(TRACKS)) {
        const match = pool.find(track => typeof track === 'object' && (track.us === name || track.pal === name));
        if (match) return match;
    }
    return undefined;
}

function resolveTrackName(name) {
    const usePal = (localStorage.getItem("locale") || "en-GB") === "en-GB";
    const pair = findRegionPair(name);
    if (!pair) return name;
    return usePal ? pair.pal : pair.us;
}

const ICON_BASE_PATH = '/_assets/media/courses/mk8dxicons';

function slugify(name) {
    return name.replaceAll(' ', '_').replaceAll("'", '').toLowerCase();
}

// US icons live in the us/ subfolder - every other track only has one icon
function getTrackIconPath(name) {
    const pair = findRegionPair(name);
    if (!pair) return `${ICON_BASE_PATH}/${slugify(name)}.avif`;

    const usePal = (localStorage.getItem("locale") || "en-GB") === "en-GB";
    const slug = slugify((usePal ? pair.pal : pair.us).replaceAll('-', ' '));
    return usePal ? `${ICON_BASE_PATH}/${slug}.avif` : `${ICON_BASE_PATH}/us/${slug}.avif`;
}

export { resolveTrackName, getTrackIconPath };
