...
function generateAllMatches(selectedSeason) {
    ...
    const rowsHTML = filteredMatches.map(match => {
        ...
        if (match.results && Array.isArray(match.results[0]) && Array.isArray(match.results[1])) {
            ...
            scoreAText = String(scoreA);
            scoreBText = String(scoreB);
        ...
        return `
            <tr class="standings-row${match.testMatch ? ' test-match-row' : ''}"${hrefAttr}>
                ...
                <td class="column-vs">
                    ...
                    ${match.testMatch ? '<span class="test-match-tag">Test match</span>' : ''}
                </td>
