...
function findLiveMatchTeam(log, date) {
    const now = new Date();
...
async function showDailyLog(date, dayCell) {
    ...
    if (log.length) {
        let liveResults = [];
        const liveTeam = findLiveMatchTeam(log, date);
        if (liveTeam) {
            try {
                liveResults = await getLiveResults(liveTeam);
            } catch (error) {
...
function generateCalendarListView() {
    ...
    matchDataToUse = overseasDateDisplay ? normalizeMatchData(matchData) : matchData;
    // Object.keys() only preserves insertion order for non-numeric-looking keys like
    // "2026-4-5" - it isn't guaranteed to be chronological, and date strings from the
    // API aren't reliably zero-padded, so sort numerically rather than lexically.
    const sortedDates = Object.keys(matchDataToUse).sort((a, b) => parseLocalDate(a) - parseLocalDate(b));
    const locale = localStorage.getItem("locale") || "en-GB";
    ...
    for (const date of sortedDates) {
        const formattedDate = parseLocalDate(date).toLocaleString(locale, { dateStyle: "long" });
        ...
        HTMLOutput += `<table class="standings-table"><tbody>`;
            ...
            HTMLOutput += `
                <tr class="standings-row${entry.testMatch ? ' test-match-row' : ''}" data-date="${date}">
                    ...
                    <td class="column-info">
                        <span class="match-season-label">${entry.testMatch ? 'Test match' : `Season ${entry.season}`}</span>
                        <span class="match-time">${isLive ? '<span class="live-dot"></span>' : ''}<span translate="no">${formattedMatchTime}</span></span>
                        ${outsideUKTimezone ? `<span class="team-date">${formattedLocalMatchTime} local</span>` : ''}
                    </td>
                    <td class="column-score"><strong>${resultsHTML || "Upcoming"}</strong></td>
                </tr>
            `;
        }
    ...
    listViewContent.querySelectorAll('.list-view-date-header').forEach(header => {
        ...
        header.addEventListener('click', () => jumpToDate(header.dataset.date));
    });
...
function animateScrollTo(el, top, duration = 350) {
    ...
    const startTime = performance.now();
    scrollAnimationStartTime = startTime;
...
document.getElementById('listViewContent')?.addEventListener('wheel', cancelListScrollAnimation, { passive: true });
document.getElementById('listViewContent')?.addEventListener('touchstart', cancelListScrollAnimation, { passive: true });
...
function scrollMatchList(smooth = false) {
    ...
    if (!target) {
        ...
        target = allHeaders.find(h => parseLocalDate(h.dataset.date) >= targetDate) || allHeaders.at(-1);
    }
    if (!target) return;

    const originalPosition = target.style.position;
    target.style.position = 'static';
    const containerRect = listViewContent.getBoundingClientRect();
    const targetRect = target.getBoundingClientRect();
    target.style.position = originalPosition;

    const targetScrollTop = listViewContent.scrollTop + (targetRect.top - containerRect.top);
    scrollTo(targetScrollTop);
    flashListDate(target);
}

// Briefly highlight the selected date's header and matches in the All Matches
// pane so it's obvious which section the calendar/list selection jumped to.
function flashListDate(header) {
    const table = header.nextElementSibling?.classList.contains('standings-table') ? header.nextElementSibling : null;
    [header, table].filter(Boolean).forEach(el => {
        el.classList.remove('date-highlight-flash');
        void el.offsetWidth;
        el.classList.add('date-highlight-flash');
        el.addEventListener('animationend', () => el.classList.remove('date-highlight-flash'), { once: true });
    });
}
...
const updateFetch = async () => {
    try {
        ...
        if (liveIndicatorDiv) {
            const liveEntry = matchDataToUse[dateParam || Object.keys(matchDataToUse)[0]]?.find(entry => entry.eventID === eventliveIndicatorToUpdate && !entry.endTime);
            if (!liveEntry) {
                liveIndicatorDiv.innerHTML = '';
            } else {
                const liveResults = await getLiveResults(liveEntry.teamsInvolved[0]);
...
async function generateMatchImage() {
    try {
        ...
        for (const node of nodes) {
            const matchBoxes = node.querySelectorAll('.timer-indicator, .youtube-box');
