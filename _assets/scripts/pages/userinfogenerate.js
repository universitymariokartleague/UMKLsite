...
const profileCardContentHTML = `
    <div class="profile-card-header">
        <img src="{{PFP}}" alt="{{username}} profile picture" class="profile-card-avatar"
            onload="this.style.opacity=1" onerror="this.onerror=null; this.src='/_assets/media/faq/defaultavatar.avif';"/>
...
const profileCardFormatHTML = `
    <div class="profile-card-wrapper">
        <div class="profile-card" style="--team-color: #{{color}};">
            <img src="/_assets/media/profile/wordmark_standard.avif" alt="UMKL logo" class="profile-umkl-logo" onload="this.style.opacity=0.9" />
...
// --- overlay ---
// Overlays not listed here fall back to an image at /_assets/media/profile/<file-name>.avif -
...
async function goBackToProfile() {
    ...
    setTimeout(() => {
        ...
        setTimeout(async () => {
            ...
            const logoHTML = `<img src="/_assets/media/profile/wordmark_standard.avif" alt="UMKL logo" class="profile-umkl-logo" onload="this.style.opacity=0.9" />`;
...
function createItemElement(item, index, isEquipped) {
    ...
    if (item.type === "colour") {
        ...
        preview.src = `/_assets/media/profile/${getItemFileName(item.name)}.avif`;
...
function applyEquippedItemsToCard() {
        ...
        if (item?.type === "overlay") {
            ...
            if (generated) {
                ...
                Object.assign(existingOverlay.style, baseStyle, {
                    backgroundImage: `url('/_assets/media/profile/${overlayFileName}.avif')`
                });
            }
        }
    }
    ...
    if (currentEquippedItems.background != null) {
        ...
        if (item?.type === "background") {
            ...
            profileCard.style.backgroundImage = `linear-gradient(rgba(255, 255, 255, ${overlayOpacity}), rgba(255, 255, 255, ${overlayOpacity})), url('/_assets/media/profile/${bgFileName}.avif')`;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const response = await fetch('/_assets/media/profile/profileunlockitems.json');
