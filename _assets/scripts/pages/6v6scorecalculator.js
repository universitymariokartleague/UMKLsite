...
function renderResults(width) {
    ...
    const teamRaceResults = positions.map((yourTeamPositions, i) => {
        ...
        return `<div class="race-row"><span class="race-name">Race ${i + 1}</span><span class="race-score">${yourTeamScore}</span></div>`;
    });
    ...
    const opponentRaceResults = positions.map((yourTeamPositions, i) => {
        ...
        return `<div class="race-row"><span class="race-name">Race ${i + 1}</span><span class="race-score">${opponentTeamScore}</span></div>`;
    });
    ...
    <details class="extra-info-box" id="extra-info-box" ${extraInfoOpened ? 'open' : ''}>
        ...
        <p class="perspective-text">
            ...
            <button id="togglePerspective" class="bubble-link">Toggle perspective</button>
        </p>
    ...
    teamResult.innerHTML = `
        <div class="result-box-header">
            <h3>${teamNames[0] || 'Your Team'}</h3>
            <span class="result-total">${yourTeamTotal}</span>
        </div>
        <div class="race-list">${teamRaceResults.join("")}</div>
        ${penalties[0] > 0 ? `<span class="diff-neg penalty-tag">Penalty: ${penalties[0]}</span>` : ""}`;

    opponentResult.innerHTML = `
        <div class="result-box-header">
            <h3>${teamNames[1] || 'Opponent Team'}</h3>
            <span class="result-total">${opponentTeamTotal - penalties[1]}</span>
        </div>
        <div class="race-list">${opponentRaceResults.join("")}</div>
        ${penalties[1] > 0 ? `<span class="diff-neg penalty-tag">Penalty: ${penalties[1]}</span>` : ""}`;
    ...
    // getComputedStyle on a custom property returns its unresolved specified value
    // (literally "var(--brand-dark)"/"var(--brand-light)"), never the actual colour -
    // read the resolved `color` on body (which uses --text-color) instead.
    const textColor = getComputedStyle(document.body).color;
    const isDarkMode = textColor === 'rgb(255, 255, 255)';
    ...
    if (width > 0) {
        chartBackgroundPlugin = {
            ...
            beforeDraw: (chart) => {
                ...
                ctx.fillStyle = isDarkMode ? "#160702" : "#FFFFFF";
